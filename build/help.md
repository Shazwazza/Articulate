...
| `--clean`         | `false`                                             | Clean source outputs, client assets, and the local test site's `umbraco` state first; this removes its database, logs, indexes, and caches. Required when switching lanes. Same-lane builds reuse client dependencies and, when tracked client inputs and build mode are unchanged, the incremental client stamp. CI preserves client dependencies and does not remove local test-site state. |
| `--preserve-site` | `false`                                             | Keep the local test site's `umbraco` state while cleaning build/client outputs; useful for testing forward migrations.                                                                                                                                                                                                              |
