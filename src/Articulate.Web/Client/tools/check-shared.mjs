...
const SHARED = [
	...
	"packages/articulate-markdown-editor/components/input-markdown-editor/input-markdown.element.ts",
...
for (const rel of SHARED) {
	...
	let a, b;
	try {
		a = readFileSync(v17, "utf8");
	} catch {
		console.error(`MISSING v17: ${rel}`);
		missing++;
		mismatches++;
		continue;
	}
	try {
		b = readFileSync(v18, "utf8");
	} catch {
		console.error(`MISSING v18: ${rel}`);
		missing++;
		mismatches++;
		continue;
	}
